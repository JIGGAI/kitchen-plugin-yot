...
describe('normalizeReferralSource', () => {
  it('preserves native options and folds their obvious variants', () => {
    ...
    expect(normalizeReferralSource('Facebook rants raves and reviews')).toBe('Facebook');
    expect(normalizeReferralSource('Instagram')).toBe('Instagram');
    expect(normalizeReferralSource('IG')).toBe('Instagram');
    expect(normalizeReferralSource('TikTok')).toBe('TikTok');
    expect(normalizeReferralSource('Tik Tok')).toBe('TikTok');
    expect(normalizeReferralSource('YouTube')).toBe('YouTube');
    expect(normalizeReferralSource('Twitter')).toBe('X');
    expect(normalizeReferralSource('X')).toBe('X');
    expect(normalizeReferralSource('Snapchat')).toBe('Snapchat');
    expect(normalizeReferralSource('LinkedIn')).toBe('LinkedIn');
    expect(normalizeReferralSource('Pinterest')).toBe('Pinterest');
    expect(normalizeReferralSource('Reddit')).toBe('Reddit');
