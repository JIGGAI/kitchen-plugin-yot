#!/usr/bin/env node
// Probe script: pulls the StaffTimeCardSummary report once and dumps its
// structure so we can write the parser against real columns. Run with:
//   node scripts/sample-staff-timecard.mjs --team-id=hmx-marketing-team
//
// Outputs:
//   logs/staff-timecard-sample-<ts>.xlsx  (raw XLSX for inspection / fixture copy)
...
const REPORT_TYPE = 'YoureOnTime.Web.TelerikReports.StaffTimeCardSummary, YoureOnTime.Reports';
const REPORT_NAME = 'StaffTimeCardSummaryReport';
const REPORT_CLASS = 'StaffTimeCardSummary';
...
const xlsxPath = join(logsDir, `staff-timecard-sample-${Date.now()}.xlsx`);
...
console.log('Next step: document column layout in scripts/sample-staff-timecard-findings.md');
